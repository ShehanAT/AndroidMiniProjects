 ...
 * maintaining an internal counter. When the counter is 0 - it is consider to be idle, when it is
...
class SimpleCountingIdlingResource(private val resourceName: String): IdlingResource {
    ...
    /**
     * Increments the count of in-flight transaction to the resource being monitored
     */
    fun increment(){
    ...
    fun decrement(){
        ...
        if(counterVal == 0){
            // we've gone from non-zero to zero. That means we're idle now! Therefore, tell Espresso
